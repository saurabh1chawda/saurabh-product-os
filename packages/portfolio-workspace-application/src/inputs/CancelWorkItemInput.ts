...
export class CancelWorkItemInput {
  private readonly __cancelWorkItemInputBrand!: never;
  ...
  equals(other: CancelWorkItemInput | undefined): boolean {
    return other instanceof CancelWorkItemInput
...
function assertInstance<T>(value: unknown, constructor: new (...args: never[]) => T): asserts value is T {
  if (!(value instanceof constructor)) {
    throw new TypeError("Invalid CancelWorkItemInput value.");
  }
}
