...
import type { LearningPlannerExplanationInput } from "../models";
import { immutableArray, immutableRecord } from "../shared";

export function createLearningPlannerExplanationSummary(input: LearningPlannerExplanationInput): ExplanationSummary {
  const references = immutableArray(input.evidenceReferenceIds.map((id) => immutableRecord({
    referenceId: id,
    referenceType: "learning-planner-evidence",
    label: id,
  ...
  const graph: ExplanationGraph = immutableRecord({
    graphId: `learning-planner-graph:${input.decisionId}`,
    nodes: immutableArray([
      immutableRecord({
        id: `learning-planner-node:${input.decisionId}`,
  ...
  const evidenceTrace: EvidenceTrace = immutableRecord({
    traceId: `learning-planner-trace:${input.decisionId}`,
