...
export function includesAlias(text: string, aliases: readonly string[]): boolean {
...
export function uniqueSorted(values: readonly string[]): readonly string[] {
  return immutableArray([...new Set(values.map((value) => value.trim()).filter(Boolean))].sort((left, right) => left.localeCompare(right)));
}
...
export function confidenceFromScore(score: number, rationale: string): Confidence {
  ...
  return createConfidence({ value, band: confidenceBand(value), rationale });
}
...
export function decisionReference(referenceId: string, referenceType: string, label: string, authority: DecisionReference["authority"] = "derived"): DecisionReference {
