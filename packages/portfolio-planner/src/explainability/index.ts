...
import type { PortfolioPlannerExplanationInput } from "../models";
import { immutableArray, immutableRecord } from "../shared";

export function createPortfolioPlannerExplanationSummary(input: PortfolioPlannerExplanationInput): ExplanationSummary {
  const references = immutableArray(input.evidenceReferenceIds.map((id) => immutableRecord({
    referenceId: id,
    referenceType: "portfolio-planner-evidence",
    label: id,
  ...
  const graph: ExplanationGraph = immutableRecord({
    graphId: `portfolio-planner-graph:${input.decisionId}`,
    nodes: immutableArray([
      immutableRecord({
        id: `portfolio-planner-node:${input.decisionId}`,
  ...
  const evidenceTrace: EvidenceTrace = immutableRecord({
    traceId: `portfolio-planner-trace:${input.decisionId}`,
  ...
  const narrative: DecisionNarrative = immutableRecord({
    narrativeId: `portfolio-planner-narrative:${input.decisionId}`,
    decisionId: input.decisionId,
    title: input.title,
    outcome: input.confidenceScore >= 50 ? "recommended" as const : "requires-review" as const,
