...
if (testDatabaseUrl === undefined || testDatabaseUrl.trim().length === 0) {
  console.error("PORTFOLIO_WORKSPACE_TEST_DATABASE_URL is required for Portfolio Workspace PostgreSQL integration tests.");
  process.exit(1);
}
...
const result = spawnSync(pnpm, [
  ...
  "vitest",
  "run",
  "tests/portfolio-workspace-postgres-live.test.ts",
  "--passWithNoTests"
], {
  cwd: packageRoot,
  env: process.env,
