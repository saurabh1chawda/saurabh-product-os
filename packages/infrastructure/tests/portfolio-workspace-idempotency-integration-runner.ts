...
if (testDatabaseUrl === undefined || testDatabaseUrl.trim().length === 0) {
  console.error("PORTFOLIO_WORKSPACE_TEST_DATABASE_URL is required for Portfolio Workspace idempotency integration tests.");
  process.exit(1);
}

try {
  assertSafePortfolioWorkspaceTestDatabaseUrl(testDatabaseUrl);
} catch (error) {
...
const result = spawnSync(pnpm, [
  ...
  "vitest",
  "run",
  "tests/portfolio-workspace-idempotency-postgres-live.test.ts",
  "--passWithNoTests"
], {
  cwd: packageRoot,
  env: process.env,
...
function assertSafePortfolioWorkspaceTestDatabaseUrl(value: string): void {
  ...
  if (!databaseName.includes("test")) {
    throw new Error("Refusing to run PostgreSQL idempotency integration tests: database name must contain 'test'.");
  }
  if (databaseName.includes("prod") || databaseName.includes("production")) {
    throw new Error("Refusing to run PostgreSQL idempotency integration tests against a database name that appears production-related.");
  }
}
