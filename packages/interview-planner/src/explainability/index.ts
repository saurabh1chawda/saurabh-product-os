...
import type { InterviewPlannerExplanationInput } from "../models";
import { immutableArray, immutableRecord } from "../shared";

export function createInterviewPlannerExplanationSummary(input: InterviewPlannerExplanationInput): ExplanationSummary {
  const references = immutableArray(input.evidenceReferenceIds.map((id) => immutableRecord({
    referenceId: id,
    referenceType: "interview-planner-evidence",
    label: id,
  ...
  const graph: ExplanationGraph = immutableRecord({
    graphId: `interview-planner-graph:${input.decisionId}`,
    nodes: immutableArray([
      immutableRecord({
        id: `interview-planner-node:${input.decisionId}`,
  ...
  const evidenceTrace: EvidenceTrace = immutableRecord({
    traceId: `interview-planner-trace:${input.decisionId}`,
  ...
  const narrative: DecisionNarrative = immutableRecord({
    narrativeId: `interview-planner-narrative:${input.decisionId}`,
    decisionId: input.decisionId,
    title: input.title,
    outcome: input.confidenceScore >= 50 ? "recommended" as const : "requires-review" as const,
