...
import type { ApplicationPlannerExplanationInput } from "../models";
import { immutableArray, immutableRecord } from "../shared";

export function createApplicationPlannerExplanationSummary(input: ApplicationPlannerExplanationInput): ExplanationSummary {
  const references = immutableArray(input.evidenceReferenceIds.map((id) => immutableRecord({
    referenceId: id,
    referenceType: "application-planner-evidence",
    label: id,
  ...
  const graph: ExplanationGraph = immutableRecord({
    graphId: `application-planner-graph:${input.decisionId}`,
    nodes: immutableArray([
      immutableRecord({
        id: `application-planner-node:${input.decisionId}`,
  ...
  const evidenceTrace: EvidenceTrace = immutableRecord({
    traceId: `application-planner-trace:${input.decisionId}`,
  ...
  const narrative: DecisionNarrative = immutableRecord({
    narrativeId: `application-planner-narrative:${input.decisionId}`,
    decisionId: input.decisionId,
    title: input.title,
    outcome: input.confidenceScore >= 50 ? "recommended" as const : "requires-review" as const,
