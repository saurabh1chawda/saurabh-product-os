...
if (testDatabaseUrl === undefined || testDatabaseUrl.trim().length === 0) {
  console.error("PORTFOLIO_WORKSPACE_TEST_DATABASE_URL is required for Portfolio Workspace HTTP host integration tests.");
  process.exit(1);
}
...
const result = spawnSync(pnpm, [
  ...
  "vitest",
  "run",
  "tests/portfolio-workspace-http-host-live.test.ts",
  "--passWithNoTests"
], {
  cwd: packageRoot,
  env: process.env,
...
function assertSafeTestDatabaseUrl(value: string): void {
  ...
  if (!databaseName.includes("test")) {
    throw new Error("Refusing to run PostgreSQL HTTP host integration tests: database name must contain 'test'.");
  }
  if (databaseName.includes("prod") || databaseName.includes("production")) {
    throw new Error("Refusing to run PostgreSQL HTTP host integration tests against a database name that appears production-related.");
  }
}
