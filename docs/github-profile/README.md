# Hi, I'm Saurabh Chawda.

I build AI products, product decision systems, and reusable frameworks that help teams make better product decisions.

This GitHub exists to share the working artifacts behind that practice.

Welcome to my Open Product Lab.

This is where I publish the frameworks, templates, decision systems, and reusable product artifacts behind [Product OS](https://saurabh-product-os.vercel.app).

## Executive Positioning

I build products and decision systems across AI, platforms, enterprise SaaS, payments, and growth.

Product OS explains how I think. GitHub shows the reusable assets behind that thinking.

Start with the [Executive Brief](https://saurabh-product-os.vercel.app/executive), continue into Product OS, inspect the flagship repositories, then contact me if the evidence maps to your role.

## Operating Principle

> Every repository should leave another Product Manager better than they arrived.

That means each public repo should offer something useful: a sharper decision model, a reusable template, a practical framework, or a clearer way to connect customer value, technical choices, and business outcomes.

## Current Focus

- AI product decision systems for prioritization, architecture, trust, and measurement.
- Platform strategy artifacts for scalability, reliability, and AI readiness.
- Product Leadership Briefs that document decisions, trade-offs, and outcomes.
- Toolkits and templates that other product teams can reuse.

## Why This GitHub Exists

This is where reusable product assets live.

Use it to inspect the implementation layer behind Product OS: frameworks, templates, examples, documentation systems, and product tools.

## Product Collections

### AI Product Collection
🧠 Use this collection to evaluate how I approach AI product opportunities.

Working artifacts focus on opportunity scoring, prioritization, architecture choices, trust, and AI success metrics.

Start here: [AI Product Playbook](https://saurabh-product-os.vercel.app/ai-product-playbook)

### Product Leadership Collection
🏗 Use this collection to understand how I make product decisions under ambiguity.

Working artifacts include Product OS implementation, Product Leadership Brief structures, decision notes, and evidence maps.

Start here: [Product Leadership Briefs](https://saurabh-product-os.vercel.app/case-studies)

### Product Toolkit Collection
🛠 Use this collection to reuse practical product artifacts.

Working artifacts include discovery templates, AI prioritization tools, decision memo templates, and product leadership brief structures.

Start here: [Product Discovery Toolkit](https://saurabh-product-os.vercel.app/decision-systems/customer-discovery)

## Start Here

| Product | Use it for |
| --- | --- |
| [Product OS](https://saurabh-product-os.vercel.app) | Executive overview of my product judgment |
| [AI Product Playbook](https://saurabh-product-os.vercel.app/ai-product-playbook) | Practical AI product operating model |
| [Product Leadership Briefs](https://saurabh-product-os.vercel.app/case-studies) | Evidence-backed product decisions |
| Product Toolkit | Reusable discovery, prioritization, decision memo, and leadership brief artifacts |

## Product OS

[Product OS](https://saurabh-product-os.vercel.app) is the executive overview.

GitHub is where the reusable assets live.

Recommended path:

1. [Executive Brief](https://saurabh-product-os.vercel.app/executive)
2. [Product OS](https://saurabh-product-os.vercel.app)
3. Flagship repositories and reusable artifacts
4. [Contact](https://saurabh-product-os.vercel.app/contact)

## Explore by Audience

### If you're a recruiter

1. [Executive Brief](https://saurabh-product-os.vercel.app/executive)
2. [Professional Profile](https://saurabh-product-os.vercel.app/profile)
3. [Case Studies](https://saurabh-product-os.vercel.app/case-studies)
4. [Contact](https://saurabh-product-os.vercel.app/contact)

### If you're a Product Manager

1. [Decision Operating System](https://saurabh-product-os.vercel.app/decision-operating-system)
2. [Operating Principles](https://saurabh-product-os.vercel.app/product-leadership-operating-principles)
3. [Customer Discovery](https://saurabh-product-os.vercel.app/decision-systems/customer-discovery)
4. [Validation & Experimentation](https://saurabh-product-os.vercel.app/decision-systems/validation-experimentation)

### If you're an Engineering Leader

1. [Logix Product Leadership Brief](https://saurabh-product-os.vercel.app/case-studies/logix)
2. [Build vs Buy AI](https://saurabh-product-os.vercel.app/decision-systems/build-vs-buy-ai)
3. [RAG vs Agent](https://saurabh-product-os.vercel.app/decision-systems/rag-vs-agent)
4. [AI Product Playbook](https://saurabh-product-os.vercel.app/ai-product-playbook)

### If you're an AI Builder

1. [AI Product Playbook](https://saurabh-product-os.vercel.app/ai-product-playbook)
2. [AI Product Principles](https://saurabh-product-os.vercel.app/ai-product-principles)
3. [AI Prioritization](https://saurabh-product-os.vercel.app/decision-systems/ai-prioritization)
4. [AI Success Metrics](https://saurabh-product-os.vercel.app/decision-systems/ai-success-metrics)

## Connect

- Product OS: [saurabh-product-os.vercel.app](https://saurabh-product-os.vercel.app)
- LinkedIn: [linkedin.com/in/chawdasaurabh](https://www.linkedin.com/in/chawdasaurabh/)
- Email: [saurabh1chawda@gmail.com](mailto:saurabh1chawda@gmail.com)

Looking for the executive overview? Start with [Product OS](https://saurabh-product-os.vercel.app).

Looking for reusable product frameworks? Explore the repositories.

Looking to discuss AI Product Management or Product Leadership? Connect on [LinkedIn](https://www.linkedin.com/in/chawdasaurabh/).
